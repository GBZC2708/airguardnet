...
- `/forgot-password` (UI; la lógica exacta de backend se implementa según HU)
...
- `RegisterPage`:
  - Según política (para este documento, dejar TODO si no se define claramente en backend).
...
- `SummaryCards`:
  ...
  - Origen:
    - `/api/usage-reports` (último registro) y/o endpoints estadísticos definidos explícitamente.

- `ChartsSection`:
  - Gráfico 24h PM2.5:
    - Llama a `/api/devices/{id}/readings` o a un endpoint de estadísticas si se define en backend.
...
- Notas de sensor (`sensor_notes`) mostradas si el backend expone esta información.
...
Notas sobre `sensor_notes`:

- La tabla `sensor_notes` existe a nivel de backend y BD.
- En esta versión la gestión (CRUD completo) de notas de sensor se considera opcional.
- Codex NO debe implementar endpoints `/api/sensor-notes` ni lógica adicional para notas de sensor a menos que previamente se actualice 02_BACKEND_ESPECIFICACION.md con el contrato exacto.
- Si en el futuro se desea mostrar/editar notas de sensor en la web, se añadirán explícitamente los endpoints correspondientes en la especificación backend.
...
- No se define por ahora un endpoint específico `/api/readings/stats`.
- Para gráficos globales o por dispositivo se usarán:
  - `/api/usage-reports` para KPIs agregados.
  - `/api/devices/{id}/readings` para series de PM2.5 por dispositivo.
- Cualquier endpoint estadístico adicional debe ser explícitamente añadido a 02_BACKEND_ESPECIFICACION.md antes de ser implementado. Codex NO debe inventar nuevos endpoints de estadísticas.
...
- Consumir exclusivamente los endpoints documentados en 02_BACKEND_ESPECIFICACION.md o añadidos explícitamente en estos documentos.
- Manejo de errores:
